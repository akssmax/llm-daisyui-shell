/** Tailwind CSS v3.4 default palette — hex values aligned with design AI prompts. */
...
export type TailwindColorRamp = {
  name: string
  shades: Record<TailwindShade, string>
}

function ramp(name: string, shades: Record<TailwindShade, string>): TailwindColorRamp {
  return { name, shades }
}

/** Gray scales first, then chromatic (alphabetical). */
export const TAILWIND_COLOR_RAMPS: TailwindColorRamp[] = [
  ramp("slate", {
    50: "#F8FAFC",
    100: "#F1F5F9",
    200: "#E2E8F0",
...
export const TAILWIND_BASE_SWATCHES = [
  { label: "white", hex: "#FFFFFF" },
  { label: "black", hex: "#000000" },
] as const

/** Flat list for search / legacy use. */
export function flattenTailwindSwatches(): Array<{ label: string; hex: string }> {
  const out: Array<{ label: string; hex: string }> = [...TAILWIND_BASE_SWATCHES]
  for (const r of TAILWIND_COLOR_RAMPS) {
    for (const shade of TAILWIND_SHADES) {
      out.push({ label: `${r.name}-${shade}`, hex: r.shades[shade] })
    }
  }
  return out
}
